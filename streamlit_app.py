...
def load_data():
    ...
    # Make sure the Excel file has columns: "Country", "Case Percentage"
    df = df.rename(columns={"Country Name": "Country"})  # adjust if needed
    return df[["Country", "Case Percentage"]]
...
for country, travelers in zip(countries, traveler_counts):
    ...
    if not row.empty:
        case_percentage = row.iloc[0]["Case Percentage"]  # already precomputed %
        estimated_cases = travelers * (case_percentage / 100)
