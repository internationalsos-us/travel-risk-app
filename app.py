...
for country, travelers in zip(countries, traveler_counts):
    ...
    if not row.empty:
        ...
        for col in case_columns:
            prob = row.iloc[0][col]
            estimated = travelers * (prob / 100)  # convert % to decimal
...
if not results_df.empty and results_df["Travelers"].sum() > 0:
    ...
    fig = px.bar(results_df, x="Country", y="Total Cases", 
                 text=results_df["Total Cases"].round(0),
    ...
    def create_pdf(dataframe, total_cases, case_totals):
        ...
        for _, row in dataframe.iterrows():
            c.drawString(30, y, f"{row['Country']}: {int(row['Total Cases'])} total cases "
        ...
        for _, row in case_totals.iterrows():
            c.drawString(30, y, f"{row['Case Type']}: {int(row['Estimated Cases'])} cases")
            y -= 20
